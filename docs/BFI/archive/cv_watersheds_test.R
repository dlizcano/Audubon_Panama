...
d3 <- d2 %>% 
  ...
  group_by(site, point, month, species, distance_bin, observer_id) %>% 
  count() 
...
d4 <- all_combos %>% left_join(d3) %>% mutate(n=ifelse(is.na(n), 0, n)) %>% 
  arrange(site, point, month, species, distance_bin) %>% 
  pivot_wider(names_from=c(observer_id, distance_bin), values_from=n)
