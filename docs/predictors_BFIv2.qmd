...
We will now examine the fit and plausibility of various models, focusing on models that contain none, one, and up to seven (i.e., all) of these possible predictors covariates.

With level = 1, we stick to models with main effects only. This implies that there are $2^8$ = 256 possible models in the candidate set to consider. Since we want to keep the results for all these models (the default is to only keep up to 100 model fits), I set confsetsize=256. With crit="aicc", we select the information criterion in this case: the AICc which is the AIC corrected by small sample size, that we would like to compute for each model and that should be used for model selection and multimodel inference.
