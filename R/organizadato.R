...
f.matrix.creator2<-function(data,year){
  ...
  for(i in 1:length(species)){
    ...
    for(j in 1:length(dates.cameras[,1])){
      col<-which(date.header==as.character( dates.cameras[j,1]))
      row<-which(cams==as.character( dates.cameras[j,2]))
      mat_h[row,col]<-hour(hours.cameras[j])
    }
    mat_h.nas<-is.na(mat)
...
umf_y_full<- unmarkedFrameOccu(y= parita[[5]]) #Actitis macularius
...
effort <- matrix(1, nrow = nrow(parita[[1]]), ncol =  ncol(parita[[1]]))

data_list <- list(ylist    = parita,
